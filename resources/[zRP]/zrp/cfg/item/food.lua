...
local function gen(ftype, vary_hunger, vary_thirst)
  local fgen = function(args)
    ...
    choices[act] = {function(player,choice)
      ...
      if user_id ~= nil then
        if zRP.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 then zRP.varyHunger(user_id,vary_hunger) end
          if vary_thirst ~= 0 then zRP.varyThirst(user_id,vary_thirst) end

          if ftype == "drink" then
            zRPclient._notify(player,"~b~ Drinking "..name..".")
            play_drink(player)
          elseif ftype == "eat" then
            zRPclient._notify(player,"~o~ Eating "..name..".")
            play_eat(player)
          end
