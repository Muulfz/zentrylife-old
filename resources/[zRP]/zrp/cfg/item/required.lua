...
items["money"] = {"Money","Packed money.",function(args)
  ...
  choices["Unpack"] = {function(player,choice,mod)
    ...
    if user_id then
      local amount = zRP.getInventoryItemAmount(user_id, idname)
      local ramount = zRP.prompt(player, "How much to unpack ? (max "..amount..")", "")
      ramount = parseInt(ramount)
      if zRP.tryGetInventoryItem(user_id, idname, ramount, true) then -- unpack the money
        zRP.giveMoney(user_id, ramount)
...
items["money_binder"] = {"Money binder","Used to bind 1000$ of money.",function(args)
  ...
  choices["Bind money"] = {function(player,choice,mod) -- bind the money
    ...
    if user_id then
      ...
      if money >= 1000 then
        if zRP.tryGetInventoryItem(user_id, idname, 1, true) and zRP.tryPayment(user_id,1000) then
          zRP.giveInventoryItem(user_id, "money", 1000, true)
          zRP.closeMenu(player)
        end
      else
        zRPclient._notify(player,zRP.lang.money.not_enough())
      end
    end
  end}
...
local get_wname = function(weapon_id)
  ...
  return zRP.lang.weapon[string.lower(name)]({}, name)
end
...
local wbody_choices = function(args)
  ...
  choices["Equip"] = {function(player,choice)
    local user_id = zRP.getUserId(player)
    if user_id then
      if zRP.tryGetInventoryItem(user_id, fullidname, 1, true) then -- give weapon body
        local weapons = {}
        weapons[args[2]] = {ammo = 0}
        zRPclient._giveWeapons(player, weapons)
...
local wammo_choices = function(args)
  ...
  choices["Load"] = {function(player,choice)
    local user_id = zRP.getUserId(player)
    if user_id then
      local amount = zRP.getInventoryItemAmount(user_id, fullidname)
      local ramount = zRP.prompt(player, "Amount to load ? (max "..amount..")", "")
      ramount = parseInt(ramount)

      local uweapons = zRPclient.getWeapons(player)
      if uweapons[args[2]] then -- check if the weapon is equiped
        if zRP.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- give weapon ammo
          local weapons = {}
          weapons[args[2]] = {ammo = ramount}
          zRPclient._giveWeapons(player, weapons,false)
          zRP.closeMenu(player)
        end
      end
    end
