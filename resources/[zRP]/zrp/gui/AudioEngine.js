    ...
    while(data = _this.mic_enc.output()){ //generate packets
      ...
      for(var i = 0; i < peers.length; i++){
        try{
          ...
          console.log("zRP-VoIP send error to player "+peers[i].player);
        }
      }
    }
  }
...
AudioEngine.prototype.setupPeer = function(peer)
{
  ...
  peer.data_channel.onopen = function(){
    $.post("http://zrp/audio",JSON.stringify({act: "voice_connected", player: peer.player, channel: peer.channel, origin: peer.origin}));
    peer.connected = true;
  }

  peer.data_channel.onclose = function(){
    $.post("http://zrp/audio",JSON.stringify({act: "voice_disconnected", player: peer.player, channel: peer.channel}));
  ...
  peer.conn.onicecandidate = function(e){
    $.post("http://zrp/audio",JSON.stringify({act: "voice_peer_signal", player: peer.player, data: {channel: peer.channel, candidate: e.candidate}}));
  }
}
...
AudioEngine.prototype.connectVoice = function(data)
{
  ...
  peer.conn.createOffer().then(function(sdp){
    $.post("http://zrp/audio",JSON.stringify({act: "voice_peer_signal", player: data.player, data: {channel: data.channel, sdp_offer: sdp}}));
...
AudioEngine.prototype.voicePeerSignal = function(data)
{
  ...
  if(data.data.candidate){ //candidate
    ...
    peer.conn.createAnswer().then(function(sdp){
      $.post("http://zrp/audio",JSON.stringify({act: "voice_peer_signal", player: data.player, data: {channel: data.data.channel, sdp_answer: sdp}}));
      peer.conn.setLocalDescription(sdp);
    });
  }
