...
-- pause
AddEventHandler("zRP:pauseChange", function(paused)
...
function tzRP.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tzRP.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- return menu state
--- opened: boolean
function tzRP.getMenuState()
  return menu_state
end

-- PROMPT

function tzRP.prompt(title,default_text)
...
function tzRP.request(id,text,time)
  SendNUIMessage({act="request",id=id,text=tostring(text),time = time})
  tzRP.playSound("HUD_MINI_GAME_SOUNDSET","5_SEC_WARNING")
end

-- gui menu events
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    zRPserver._closeMenu(data.id)
  elseif data.act == "valid" then
    zRPserver._validMenuChoice(data.id,data.choice,data.mod)
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    zRPserver._promptResult(data.result)
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    zRPserver._requestResult(data.id,data.ok)
  end
end)
...
function tzRP.announce(background,content)
...
RegisterNUICallback("init",function(data,cb) -- NUI initialized
  ...
  TriggerEvent("zRP:NUIready")
end)

-- PROGRESS BAR

-- create/update a progress bar
function tzRP.setProgressBar(name,anchor,text,r,g,b,value)
...
function tzRP.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tzRP.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tzRP.removeProgressBar(name)
...
function tzRP.playAudioSource(url, volume, x, y, z, max_dist)
...
--- x,y,z: position (omit for unspatialized)
--- max_dist  (omit for unspatialized)
function tzRP.setAudioSource(name, url, volume, x, y, z, max_dist)
...
function tzRP.removeAudioSource(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    local fx,fy,fz = tzRP.getCamDirection()
...
function tzRP.disconnectVoice(channel, player)
...
function tzRP.registerVoiceCallbacks(channel, on_offer, on_connect, on_disconnect)
  if not channel_callbacks[channel] then
    channel_callbacks[channel] = {on_offer, on_connect, on_disconnect}
  else
    print("[zRP] VoIP channel callbacks for <"..channel.."> already registered.")
  end
end

-- check if there is an active connection
function tzRP.isVoiceConnected(channel, player)
...
function tzRP.isVoiceConnecting(channel, player)
  local channel = voice_channels[channel]
  if channel then
...
function tzRP.getVoiceChannels()
...
--- active: true/false 
function tzRP.setVoiceState(channel, player, active)
...
----- gain => { gain: ... }
function tzRP.configureVoice(channel, config)
...
RegisterNUICallback("audio",function(data,cb)
  ...
  elseif data.act == "voice_peer_signal" then
    zRPserver._signalVoicePeer(data.player, data.data)
  end
end)

-- receive voice peer signal
function tzRP.signalVoicePeer(player, data)
...
function tzRP.isSpeaking()
  return speaking
end

if cfg.zrp_voip then -- setup voip world channel
  -- world channel behavior
  tzRP.registerVoiceCallbacks("world", function(player)
    print("(zRPvoice-world) requested by "..player)
  ...
  function(player, is_origin)
    print("(zRPvoice-world) connected to "..player)
    tzRP.setVoiceState("world", nil, speaking)
  end,
  function(player)
    print("(zRPvoice-world) disconnected from "..player)
  end)

  AddEventHandler("zRP:NUIready", function()
    -- world channel config
    tzRP.configureVoice("world", cfg.world_voice_config)
  end)
end
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("zRP:pauseChange", paused)
