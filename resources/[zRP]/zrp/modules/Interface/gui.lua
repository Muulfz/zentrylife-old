...
local cfg = module("cfg/Modules/gui")
...
function zRP.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  zRPclient._openMenuData(source, menudata)
end

-- force close player menu
function zRP.closeMenu(source)
  zRPclient._closeMenu(source)
end
...
-- return entered text
function zRP.prompt(source,title,default_text)
  local r = async()
  prompts[source] = r

  zRPclient._prompt(source, title,default_text)
...
function zRP.request(source,text,time)
  local r = async()
  ...
  zRPclient.request(source,id,text,time) -- send request to client
...
function zRP.registerMenuBuilder(name, builder)
...
function zRP.buildMenu(name, data)
  local r = async()
...
function zRP.openMainMenu(source)
  local menudata = zRP.buildMenu("main", {player = source})
...
function tzRP.validMenuChoice(id,choice,mod)
  local source = source
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tzRP.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tzRP.requestResult(id,ok)
    ...
    for k,v in pairs(cfg.static_menus) do
      ...
      if smenu then
        local function smenu_enter(source)
          local user_id = zRP.getUserId(source)
          if user_id ~= nil and zRP.hasPermissions(user_id,smenu.permissions or {}) then
            -- build static menu
            local menu = zRP.buildMenu("static:"..k, {player=source})
            menu.name=v.title
        ...
        zRPclient._addBlip(source,x,y,z,smenu.blipid,smenu.blipcolor,smenu.title)
        zRPclient._addMarker(source,x,y,z-1,0.7,0.7,0.5,255,226,0,125,150)

        zRP.setArea(source,"zRP:static_menu:"..k,x,y,z,1,1.5,smenu_enter,smenu_leave)
      end
    end
  end
end
...
AddEventHandler("zRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    zRPclient._setDiv(source,"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,"")
...
AddEventHandler("zRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    -- send peer config
    zRPclient._setPeerConfiguration(source, cfg.voip_peer_configuration)
  end
end)
