...
-- name = partid
function zRP.openSkinshop(source,parts)
  ...
  if user_id then
    ...
    local old_custom = zRPclient.getCustomization(source)
    ...
    for k,v in pairs(parts) do -- for each part, get number of drawables and build menu
      ...
      async(function()
        ...
        textures[k][2] = zRPclient.getDrawableTextures(source,v,drawables[k][1]) -- set max
      end)
    ...
    menudata.onclose = function(player)
      -- compute price
      local custom = zRPclient.getCustomization(source)
      local price = 0
      ...
      if zRP.tryPayment(user_id,price) then
        ...
        -- revert changes
        zRPclient._setCustomization(source,old_custom)
      end
    end
...
local function build_client_skinshops(source)
  ...
  if user_id then
    for k,v in pairs(skinshops) do
      ...
      zRPclient._addBlip(source,x,y,z,73,3,lang.skinshop.title())
      zRPclient._addMarker(source,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)

      zRP.setArea(source,"zRP:skinshop"..k,x,y,z,1,1.5,skinshop_enter,skinshop_leave)
    end
  end
end
