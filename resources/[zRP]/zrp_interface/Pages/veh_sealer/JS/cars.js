...
function onClickEditSaleCarBtn(id) {
    $.post('http://zrp_interface/veh_sealer/getSaleVehicle', JSON.stringify({model: id})).done(function (data) {
        swal({
            ...
            cancelButtonColor: '#FF586B',
            html: `
             <input maxlength = "9" placeholder = "Preco" value = "${data.price}" id = "input-price" class="swal2-input" onkeypress = "return event.charCode > 47 && event.charCode < 58;" pattern = "[0-9]{5}" required>
                                        ...
                                        <div class = "used-car">
                                            <div class = "used-car-container">
                                                ...
                                                <div class = "car-container">
                                                    <p>${data[1][i].name}</p>
                                                    <p class = "price">R$: ${data[1][i].price}</p>
                                                    <button onclick = "${data[1][i].owner_id ? "onClickSaleCarBtn(this.id)" : "onClickEditSaleCarBtn(this.id)"}" id = "${data[1][i].owner_id ? data[1][i].model + "|" + data[1][i].owner_id: data[1][i].model}" class="btn car-btn ${data[1][i].owner_id ? "btn-success" : "btn-warning"}">${data[1][i].owner_id ? "Comprar" : "Editar"}</button>
                                                </div>
                                            </div>    
                                            <div class = "used-car-desc">${data[1][i].description}</div>
                                        </div>
...
function onClickSellBtn(id) {
    swal({
        ...
        cancelButtonColor: '#FF586B',
        html: `
            <input maxlength = "9" placeholder = "Preco" id = "input-price" class="swal2-input" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" required>
